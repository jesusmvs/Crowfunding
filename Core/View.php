...
class View
{
     ...
     * @param  [String]  [template name], [String] [view_target_type]
     * @return [html]    [render html]
     */
    public static function render($template, $type)
    {
        ...
        ob_start();
        extract(self::$data);
        //include (self::VIEWS_PATH . 'Elements/' . $type . '/header.' . self::EXTENSION_TEMPLATES);
        include(self::VIEWS_PATH . $template . "." . self::EXTENSION_TEMPLATES);
        //include (self::VIEWS_PATH . 'Elements/' . $type . '/footer.' . self::EXTENSION_TEMPLATES);
        $str = ob_get_contents();
        ob_end_clean();
        echo $str;
    }
